...
# class PyramidMultiLabelNestIndexing(PyramidNestIndexing):
    
#     def multi_tag_indexing(self, labels):
#         # labels (B,) of (ett_type, ett_start, ett_end, length)
#         rets = []
#         for etts, length in labels:
#             print(length)
#             ret = np.zeros([length, self.config.tag_vocab_size])
#             for ett_type, ett_start, ett_end in etts:
#                 ret[ett_start+1:ett_end, self.tag_indexing.token2idx(f"I-{ett_type}")] = 1.
#                 ret[ett_start, self.tag_indexing.token2idx(f"B-{ett_type}")] = 1.
            
#         rets = pad_sequences(rets, dtype='float', padding='post', truncating='post', value=0)
#         return rets
        
#     def __call__(self, labels):
#         labels = np.array(labels) # (B, P) of List(ett_type, ett_start, ett_end, length)
#         ret = []
#         for i in range(labels.shape[1]):
#             ret.append(self.multi_tag_indexing(labels[:, i]))
#         return ret